...
# Configuration
DOMAIN="bangkok26.com"
S3_BUCKET="bangkok26-assets"
APP_NAME="bangkok26-static-site"
GITHUB_REPO="${GITHUB_REPO:-ezzy4me/bangkok26}"
GITHUB_TOKEN="${GITHUB_TOKEN}"  # Environment variable
REGION="us-east-1"
IMAGE_DIR="${BANGKOKAGENT_REF_DIR:-/Users/sangmin/Desktop/Claude/Projects/bangkokagent/Ref}"
...
if [[ "$GITHUB_REPO" == *"your-username"* ]]; then
  ...
  echo "Please set: export GITHUB_REPO=your-username/bangkok26"
...
if [ ! -d "public" ] || [ -z "$(ls -A public 2>/dev/null)" ]; then
  ...
  echo "Run ./scripts/sync-from-bangkokagent.sh first"
  exit 1
fi
...
if [ -n "$SNS_TOPIC_ARN" ]; then
  ...
  aws cloudwatch put-metric-alarm \
    --alarm-name "bangkok26-traffic-spike" \
    --comparison-operator GreaterThanThreshold \
...
echo "   - Create Hosted Zone for bangkok26.com in Route 53"
